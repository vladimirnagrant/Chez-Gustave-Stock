...
const DEFAULT_USERS = [
  { id: "u1", name: "Vladimir",        role: "employe" },
...
async function loadList(key, fallback) {
  try {
    ...
    return undefined; // signal failure, caller should keep existing state
  }
}
...
export default function StockApp() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setPins(p ?? DEFAULT_PINS);
      setCatalogue(c ?? DEFAULT_CATALOGUE);
      setRequests(r ?? []);
      setHistory(h ?? []);
      setCashRequests(m ?? []);
      setReady(true);
  ...
  async function addRequest(articleId, quantite) {
    const newReq = { id: uid(), articleId, quantite, par: currentEmployee, date: new Date().toISOString() };
...
function EmployeeView({ catalogue, onAddRequest, allRequests, history, employees, onToggleReceived, onUpdateComment, onUpdateHistoryComment, onRemoveRequest, onUpdateQty, currentEmployee, currentUser, pins, onChangePin, myRequests, cashRequests, onAddCashRequest, onToggleCashDelivered, onRemoveCashRequest }) {
  ...
  const [selectedGroupe, setSelectedGroupe] = useState(null); // "Alimentaire" | "Non Alimentaire"
          ...
          <div className="space-y-2">
            {myRequests.map((r) => {
              ...
              return (
                ...
                >
                  <span className="text-stone-700">{item ? item.name : "Article supprimé"}</span>
        ...
        <div className="space-y-4">
          {confirmRemoveId && (
            <ConfirmDialog
              ...
              message="Cet article sera retiré du panier commun."
              onCancel={() => setConfirmRemoveId(null)}
              onConfirm={() => { onRemoveRequest && onRemoveRequest(confirmRemoveId); setConfirmRemoveId(null); }}
            />
          )}
                  ...
                  <div key={r.id} className="px-3 py-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="flex-1 text-stone-700">
                        ...
                        {r.item ? r.item.name : "Article supprimé"}
                      </span>
                      <div className="flex items-center gap-1">
                        {editQtyId === r.id ? (
                          <>
                            <button onClick={() => setEditQtyVal((v) => String(Math.max(1, Number(v) - 1)))} className="w-6 h-6 rounded bg-stone-100 text-stone-600 flex items-center justify-center text-xs">−</button>
                            <input type="number" min="1" value={editQtyVal} onChange={(e) => setEditQtyVal(e.target.value)} className="w-10 border border-stone-200 rounded px-1 py-0.5 text-center text-xs" />
                            <button onClick={() => setEditQtyVal((v) => String(Number(v) + 1))} className="w-6 h-6 rounded bg-stone-100 text-stone-600 flex items-center justify-center text-xs">+</button>
                            <button onClick={() => { onUpdateQty && onUpdateQty(r.id, Number(editQtyVal)); setEditQtyId(null); }} className="text-emerald-600 text-xs font-medium px-1">✓</button>
                            <button onClick={() => setEditQtyId(null)} className="text-stone-400 text-xs px-1">✕</button>
                          </>
                        ) : (
                          <>
                            <span className="text-stone-400 text-xs">x{r.quantite}</span>
                            <button onClick={() => { setEditQtyId(r.id); setEditQtyVal(String(r.quantite)); }} className="text-amber-600 text-xs px-1">✏️</button>
                            <button onClick={() => setConfirmRemoveId(r.id)} className="text-stone-300 hover:text-red-500"><Trash2 size={13} /></button>
                          </>
                        )}
                      </div>
                    </div>
                    <div className="text-[11px] text-stone-400 pl-5">par {r.par}</div>
                    <div className="pl-5 mt-1">
                      {r.commentaire ? (
                        <button onClick={() => setCommentOpenId(commentOpenId === r.id ? null : r.id)} className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1 inline-flex items-center gap-1">
                          💬 {r.commentaire}
                        </button>
                      ) : (
                        <button onClick={() => setCommentOpenId(commentOpenId === r.id ? null : r.id)} className="text-xs text-stone-400 underline">+ Ajouter un commentaire</button>
                      )}
                      {commentOpenId === r.id && (
                        <CommentEditor
                          value={r.commentaire || ""}
                          onSave={(text) => { onUpdateComment && onUpdateComment(r.id, text); setCommentOpenId(null); }}
...
function HistoryByDate({ history, catalogue, employees, onToggleReceived, onUpdateComment, currentIdentity, mode }) {
  ...
  function handleCheckClick(a) {
    // Always pass current identity; the parent decides if it's a first receipt or a correction
    ...
    <div className="space-y-5">
      ...
      {filtered.length === 0 ? (
        ...
        sortedDays.map(([dayLabel, { articles }]) => {
          ...
          return (
            <div key={dayLabel}>
              <div className="text-xs font-medium text-amber-700 uppercase tracking-wide mb-2 capitalize">
                {dayLabel}
              </div>
                      ...
                      {items.map((a) => (
                        <div key={a.id} className="flex items-start gap-2 text-sm py-0.5">
                          <button
                            ...
                            className={`w-5 h-5 mt-0.5 rounded-md border flex items-center justify-center shrink-0 transition-colors ${
                              a.recu ? "bg-emerald-600 border-emerald-600" : "border-stone-300 bg-white"
                            }`}
                          >
                            {a.recu && <Check size={13} className="text-white" />}
                          </button>
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              <span className={a.recu ? "text-stone-700" : "text-stone-700"}>
                                {a.item?.code ? `${a.item.code} · ` : ""}
                                {a.item ? a.item.name : "Article supprimé"}
                              </span>
