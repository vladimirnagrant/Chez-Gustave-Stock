...
export default function StockApp() {
  ...
  useEffect(() => {
    (async () => {
      // Load all keys — if a key doesn't exist in Supabase yet, save the default immediately
      async function loadOrInit(key, fallback) {
        const res = await loadList(key, null);
        if (res !== null && res !== undefined) return res;
        // Not in Supabase yet — save default so future app updates don't reset it
                  ...
                  <div key={r.id} className="px-3 py-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        <button onClick={() => { onUpdateQty && onUpdateQty(r.id, Math.max(1, r.quantite - 1)); }} className="w-7 h-7 rounded-lg bg-stone-100 text-stone-600 font-bold flex items-center justify-center text-sm hover:bg-amber-100">−</button>
                        <span className="text-stone-700 font-medium text-sm w-6 text-center">{r.quantite}</span>
                        <button onClick={() => { onUpdateQty && onUpdateQty(r.id, r.quantite + 1); }} className="w-7 h-7 rounded-lg bg-stone-100 text-stone-600 font-bold flex items-center justify-center text-sm hover:bg-amber-100">+</button>
                        <button onClick={() => setConfirmRemoveId(r.id)} className="ml-1 text-stone-300 hover:text-red-500"><Trash2 size={13} /></button>
                      </div>
                    </div>
